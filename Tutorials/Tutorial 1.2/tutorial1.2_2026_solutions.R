# title: 'Learning from Big Data: Tutorial 1.2 (Solutions)'
...
x_lower = x[1:round((1/2)*length(x))]
# number of topics in Word2Vec
total_topics_word2vec <- 10

# Train the w2v model
model <- word2vec(x = x_lower, type = "cbow", dim = total_topics_word2vec, iter = 20)
...
x_upper = x[round((1/2)*length(x)):length(x)]
total_reviews = length(x_upper)
...
for (k in 1:total_reviews)
{
   ...
   tokenized_review <- unlist(strsplit(x_upper[k],"[^a-zA-Z0-9]+"))
...
log_BO   <- log(as.numeric(gsub(",", "",reviews_raw$first_week_box_office[round((1/2)*length(x)):length(x)])))
